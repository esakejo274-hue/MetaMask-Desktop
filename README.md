<img alt="MetaMask logo" src="https://i.imgur.com/XpYAuze.png?raw=true" width="50" />

# MetaMask Desktop

MetaMask Desktop is a native desktop application by ConsenSys that gives users full control over their crypto assets without any intermediaries. Unlike the browser extension, the desktop version runs independently — it doesn't rely on a browser, launches faster, and is better suited for day-to-day Web3 work.

- Security — private keys never leave your device. No custodial storage, no servers with access to your funds.
- Convenience — a single entry point for DeFi, NFTs, staking, and any dApp. The built-in swap cuts out aggregator fees.
- Flexibility — works with any EVM-compatible network, supports hardware wallets and multiple accounts.


## Wallet management
Create and import unlimited wallets. Your seed phrase stays on your device — always.

## Send & receive
Transfer tokens and NFTs between addresses with real-time transaction status tracking.

## Built-in swap
Exchange tokens at the best rate across DEX aggregators — without leaving the app.

## dApp browser
Connect to DeFi platforms, NFT marketplaces, and Web3 services in one click.

## Multi-network
Switch networks manually or let the app switch automatically when connecting to a dApp.

## Transaction signing
Review every request in full detail before signing — complete transparency, no surprises.

## Hardware wallets
Native integration with Ledger and Trezor for offline private key storage.

## Portfolio tracker
Aggregated token and NFT balances across all wallets and networks in one screen.
